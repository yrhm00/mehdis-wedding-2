...
const isFirebaseConfigured = !Object.values(firebaseConfig).some(function(value) {
  return String(value).startsWith('VOTRE_');
});
...
function submitToFirebase(nom, present, nombrePersonnes) {
  ...
  if (!db) {
    showFormError('La base de données n’est pas encore configurée. Réessayez plus tard.');
  ...
  .catch(function (error) {
    ...
    showFormError('La confirmation n’a pas pu être envoyée. Merci de réessayer.');
  })
  .finally(function () {
    submitBtn.textContent = 'Confirmer ma présence';
    submitBtn.disabled = false;
